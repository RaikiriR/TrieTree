...
public class Trie2 {
	...
	{
		TrieNode answer = goTo(root,p);
		//printSorted(answer,"");
		LinkedList h = new LinkedList();
		fillLinkedList(h,answer,p);
		return h;
	}
	
	//This sets the instance of the tree node root to the substringed tree
	private TrieNode goTo(TrieNode root,String s)
	{
		if(s == "")
		{
			return root;
	...
	//recursively fills the linked list with all of the possible words in the tree
	private void fillLinkedList(LinkedList t,TrieNode node, String s) {
		if (node.aword)
		{
			t.add(s);
		}
		for (Character ch : node.children.keySet()) {
			 fillLinkedList(t,node.children.get(ch), s + ch);
		}
	}
